...
* **Main Contribution:** The paper establishes that Transformer attention dynamics exhibit global clustering (asymptotic collapse to a single point) for almost all initializations, connects these dynamics to Wasserstein gradient flows and synchronization models, and characterizes metastable multi-cluster states and phase transitions in long-context settings.  
...
* **Prior Work:**  
    * *Previous Best Result:* For \(d=2\) (Kuramoto model), synchronization was known for \(\beta = 0\) [Taylor, 2012]. In higher dimensions, [Markdahl et al., 2017] proved clustering for specific interaction kernels.  
    * *The Gap:* Prior work did not address metastability, quantitative clustering rates, normalization effects, or phase transitions in long-context regimes. The extension to \(d=2\) for \(\beta > 0\) was unresolved.
...
* **Formal Statements:**  
    ...
    - **Theorem 2 (Exponential Local Rates):** If tokens lie in a common open hemisphere initially, \(\|x_i(t) - x^*\| \leq C e^{-\lambda t}\) for SA/USA.  
...
* **Examples & Special Cases:**  
    - **Equiangular Model:** All \(\langle x_i, x_j \rangle = \rho(t)\) for \(i \neq j\). Reduces dynamics to an ODE for \(\rho(t)\):  
...
* **Lesson:** The equiangular model isolates clustering mechanics; logarithmic \(\beta_n\) scaling is critical for stable long-context attention.  
...
* **Optimality & Counterexamples:**  
    ...
    - **Assumptions:** Clustering fails if \(d=2\) and \(\beta < -0.16\) [Andrew et al., 2025]. For large \(\beta\), mean-field dynamics may converge to multiple clusters [Chen et al., 2025].  
