...
* **Prior Work:**  
  - *Previous Best Result:* For \(d \geq 3\), Markdahl et al. [2017] proved clustering under smooth interaction kernels, while Criscitiello et al. [2024] established synchronization on spheres—both qualitative results without rates. For \(d=2\) (Kuramoto model), synchronization was known for \(\beta=0\).  
  - *The Gap:* Prior work lacked 1) quantitative convergence rates, 2) analysis of metastability and saddle transitions, 3) connections to Transformer-specific phenomena (normalization effects, long-context phase transitions).  
...
* **Formal Statements:**  
  ...
  - **Theorem 4 (Pairwise merging at large \(\beta\)):** For \(\beta \to \infty\) and equiangular initial clusters, the closest pair merges along their geodesic in rescaled time, while others remain stationary.  
* **Heuristics & Intuition:** The softmax kernel amplifies attraction between similar tokens, driving collapse. Normalization (SA) slows contraction vs. unnormalized dynamics (USA). At large \(\beta\), dynamics resemble a hardmax rule, merging clusters sequentially.  
...
* **Lessons:**  
  - Normalization (e.g., Pre-LN vs. Post-LN) critically affects contraction speed (polynomial vs. exponential).  
  - Logarithmic scaling \(\beta_n \sim \log n\) is necessary to avoid attention collapse in long sequences.  
* **Conjectures:** The number of metastable clusters scales as \(\Theta(\sqrt{\beta})\) in mean-field dynamics (supported by linearization and Kac–Rice arguments).  

---

### 5. Anatomy of the Proofs & Machinery
* **Crucial Insight:**  
  - For finite particles: Analyticity of \(\mathcal{E}_\beta\) and Łojasiewicz inequality imply convergence; center-stable manifold theorem excludes saddle attraction.  
...
* **Ingredients:**  
  ...
  - Time-rescaling in the \(\beta \to \infty\) limit (Theorem 4) to isolate pairwise mergers.  
* **Proof Skeleton:**  
  1. **Clustering:** Show critical points of \(\mathcal{E}_\beta\) are unstable except the single cluster (via Łojasiewicz and center-stable manifold theorem).  
  2. **Rates:** Grönwall inequality in hemispheres; entropy methods for mean-field.  
  3. **Metastability:** Rescale time to expose saddle-to-saddle transitions (Theorem 4).  
...
* **Optimality & Counterexamples:**  
  ...
  - **Counterexample:** For large \(\beta\), mean-field dynamics can converge to multiple clusters (Example 2.6 in [Chen et al. 2025]).  
* **Technical Obstructions:**  
  - **\(d=2\):** Proofs for \(d \geq 3\) fail due to the circle’s topology (\(\pi_1(\mathbb{S}^1) \neq 0\)); resolved for \(\beta > -0.16\) in [Andrew25].  
