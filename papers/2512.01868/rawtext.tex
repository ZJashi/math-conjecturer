...
% The paper is organized as follows. In Section~\ref{sec:original_transformer} we briefly recall the original Transformer architecture, emphasizing the mathematical structure of the attention mechanism. Section~\ref{sec:simplified_models} introduces simplified continuous-time models that retain the essence of attention and normalization, notably the Self-Attention (SA) and Unnormalized Self-Attention (USA) flows, and discusses their variational interpretation as Wasserstein gradient flows. In Section~\ref{sec:clustering}, we describe the emergence of clustering in these systems, empirically, qualitatively, and quantitatively, and establish connections with synchronization phenomena such as the Kuramoto model. Section~\ref{sec:mean_field} studies the mean-field limit and rates of convergence using recent results on interacting particle systems. Sections~\ref{sec:equiangular} and~\ref{sec:metastability} investigate the behavior under symmetric initial conditions and the appearance of metastable states, respectively, with links to Allen--Cahn dynamics and saddle-to-saddle transitions. Finally, Section~\ref{sec:noisy_transformers} introduces stochastic perturbations and discusses propagation of chaos and bifurcation phenomena in noisy Transformer dynamics.
...
\begin{theorem}[\cite{chen2025critical}]
