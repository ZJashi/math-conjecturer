...
def brainstormer_node(state: Phase2State) -> Dict[str, Any]:
    ...
    if current_proposal and feedback:
        ...
        chain = prompt | model.with_structured_output(ProposalResult)

        result = chain.invoke({
            "current_proposal": current_proposal,
            "feedback": feedback.get("overall_assessment", ""),
...
"""
    ...
    if arxiv_id:
        proposal_dir = PAPERS_DIR / arxiv_id / "step4_phase2" / "proposals"
        proposal_dir.mkdir(parents=True, exist_ok=True)
        proposal_path = proposal_dir / f"iteration_{iteration}.md"
