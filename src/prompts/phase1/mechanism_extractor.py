...
6.  **Known-false:** If the paper explicitly shows a desired behavior is false (counterexample, negative theorem, lower bound, matching upper bound, or remark), add `<known_false>` to that dissatisfaction. Do NOT add this element when the paper is merely silent or uncertain — only when it actively disproves the claim. **Lower bounds count:** if the paper proves Ω(f(d)/κ), then "remove f(d) entirely" is false. **Upper bounds count:** if the paper proves a matching upper bound showing a constant/rate is already tight, then "improve beyond that bound" is false. Add `<known_false>` in both cases.
7.  **Granular dissatisfactions:** For each theorem result, create a *separate* `<dissatisfaction>` for each independently-improvable aspect. Do NOT bundle "reduce the constant" and "remove the growth factor (e.g., log d)" into one node — they are distinct conjectures with potentially different statuses (one open, one false). For example, if a bound is `C log(d) / κ`, create two dissatisfactions: one for "reduce constant C" and one for "remove log d to get dimension-free bound C/κ." Then apply Rule 6 independently to each.
